...
#define cantidadOpcionesAstro 5
#define cantTiposNave         3
#define cantEstados           4
#define minimaCantidadOpciones 0
#define maxCantidadOpcionesMain 3
#define maxCantidadModificaciones 10
...
void funcionAstronautas(){
    ...
    while(strcmp(valor, "si") == 0)
    {
       ...
       while(operador < 0)
    {
        operador = validacionNumeros(strOpcion,minimaCantidadOpciones,cantidadOpcionesAstro);

        if (operador > minimaCantidadOpciones && operador <= cantidadOpcionesAstro)
        {
            break;
        }
    }
...
void modificarAstronauta()
{
  ...
  char valor[] = "si";
   ...
   char strOpcion[maxCaracteres];
    ...
    while(operador < 0)
    {
        operador = validacionNumeros(strOpcion,minimaCantidadOpciones,maxCantidadModificaciones);

        if (operador > minimaCantidadOpciones && operador < maxCantidadModificaciones)
        {
            break;
        }
    }
...
void cargarTipoNave(nave* nav)
{
    ...
    int valido = validacionTipoNave(seleccion);

    if(valido == 1){

        strcpy(nav->tipoNave, tiposNave[seleccion - 1]);
        ...
        while (valido == 0){
            ...
            valido = validacionTipoNave(seleccion);
        }
        strcpy(nav->tipoNave, tiposNave[seleccion - 1]);
    }
}
...
int validacionTipoNave (int seleccion)
{
    ...
    if(seleccion <= cantTiposNave && seleccion > 0)
    {
...
int validacionEstado(int seleccion)
{
    ...
    if(seleccion <= cantEstados && seleccion > 0)
    {
...
void cargarEstado(nave* nav)
{
    ...
    if (valido == 0){
        ...
        strcpy(nav->estado, estados[seleccion - 1]);
        ...
        while(valido == 0){
            ...
            printf("Igrese una opcion valida\n");
            fflush(stdin);
            scanf("%i", &seleccion);
...
nav1.ID = 1001;
strcpy(nav1.tipoNave, "Starship");
strcpy(nav1.estado, "Mantenimiento");
nav1.cantVuelosRealizados = 7;
nav1.horasAcumuladasVuelo = 106;

fwrite(&nav1, sizeof(nave), 1, archi);


//--.-.-.-.--.-.-.-.-.-.--.-.-.-.-.-.-.-.
nav2.ID = 1002;
strcpy(nav2.tipoNave, "Falcon 9");
strcpy(nav2.estado, "de Baja");
nav2.cantVuelosRealizados = 2;
nav2.horasAcumuladasVuelo = 25;

fwrite(&nav2, sizeof(nave), 1, archi);


//--.-.-.-.--.-.-.-.-.-.--.-.-.-.-.-.-.-.
nav3.ID = 1003;
strcpy(nav3.tipoNave, "Falcon Heavy");
strcpy(nav3.estado, "Mision");
nav3.cantVuelosRealizados = 10;
nav3.horasAcumuladasVuelo = 181;

fwrite(&nav3, sizeof(nave), 1, archi);
...
void mostrarArchivoNaves()
{
    ...
    while((fread(&nav, sizeof(nave), 1, arhi) > 0)
    {
